  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post(
        "http://127.0.0.1:8000/intern/register",
        formData
      );
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        time_in:
          formData.time_in.length === 5
            ? `${formData.time_in}:00`
            : formData.time_in,
        time_out:
          formData.time_out.length === 5
            ? `${formData.time_out}:00`
            : formData.time_out,
      ...
      const response = await axios.patch(
        "http://localhost:8000/intern/update",
        payload
      );
    ...
    if (min && max) {
      if (min < max) {
        ...
        if (value < min || value > max) {
          alert(
            `Time In must be between ${to12HourFormat(
              min
            )} and ${to12HourFormat(max)} for the selected shift.`
          );
            ...
            <div className="absolute -top-8 left-0 z-10 flex gap-2">
              {uniqueSchools.map((school) => (
                <button
                  ...
                  style={{
                    borderColor:
                      activeTab === school ? "#e5e7eb" : "transparent",
                  }}
                >
                  {school}
                </button>
            ...
            <div className="relative bg-white shadow-[0_2px_8px_rgba(0,0,0,0.1)] rounded-tl-none rounded-2xl px-4 z-0 overflow-visible p-4">
              {(() => {
                ...
                const filteredInterns = interns.filter(
                  (intern) =>
                    intern.abbreviation.toLowerCase() ===
                ...
                if (filteredInterns.length === 0) {
                  return (
                    <div className="flex items-center justify-center h-full">
                      <p className="text-gray-300 text-lg font-normal p-5">
                        No existing intern.
                      </p>
                    </div>
                  );
                ...
                return paginatedInterns.map((intern, index) => (
                  <div
                    ...
                    className="grid grid-cols-[3fr_2.5fr_1fr] items-center bg-white shadow-[0_2px_8px_rgba(0,0,0,0.1)] rounded-lg p-4 m-4 px-6 border-2"
                  >
                    {/* Intern per line */}
                    <div className="px-10">
                      <p className="font-bold text-[#0D223D] text-left">
                        {intern.intern_name}
                      </p>
                      <div className="flex items-center gap-1 text-sm text-gray-600">
                        ...
                        <span className="ml-1.5 px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-[10px]">
                          {intern.abbreviation}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col text-[14px] text-gray-600 pl-14 pr-2">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>
                            {intern.time_in} - {intern.time_out}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span>
                          {(intern.total_hours - intern.time_remain).toFixed(2)}/{intern.total_hours} hours
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3 justify-end px-2">
                      <div className="relative group flex items-center">
                        ...
                        {/* Tooltip */}
                        <span className="absolute top-full mt-2 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100  bg-gray-700 text-white text-xs rounded py-1 px-2 pointer-events-none transition-opacity duration-200 whitespace-nowrap">
                          Download QR Code
                        </span>
                      </div>
                      {/* Actions */}
              ...
              <div className="bg-white p-6 rounded-xl w-full max-w-xl relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex justify-end gap-2 mt-6">
                    ...
                    <button
                      type="button"
                      onClick={() => setAddInternModal(false)}
                ...
                {response?.qr_code_path && (
                  <div className="mt-4 p-3 bg-green-100 border border-green-400 rounded">
                    <p>
                      <strong>Intern Registered!</strong>
                    </p>
